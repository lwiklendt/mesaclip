...
def main():
    ...
    print(np.log2(freqs[1]) - np.log2(freqs[0]))
    ...
    gridx, gridy = np.meshgrid(log2_freq_edges, utils.make_edges(threshes))
    ...
    ax_w = fig.add_subplot(position=[ax_xs[1], ax_ys[0], ax_ws[1], ax_hs[0]])
    ax_p = fig.add_subplot(position=[ax_xs[1], ax_ys[1], ax_ws[1], ax_hs[1]])
    ...
    for i in range(n):
        ...
        ax_w = fig.add_subplot(position=[ax_xs[c+1], ax_ys[r+0], ax_ws[c+1], ax_hs[r+0]])
        ax_p = fig.add_subplot(position=[ax_xs[c+1], ax_ys[r+1], ax_ws[c+1], ax_hs[r+1]])
        axs = [ax_x, ax_p, ax_w]
        ax_fs = [ax_p, ax_w]

        # plot signals
        ax_x.plot(t, xs[i], lw=1)

        # plot peak frequency estimate
        ax_p.pcolormesh(gridx, gridy, peak_hists[i], cmap='Blues', vmin=-0.1)

        # plot wavelet
        ax_w.fill_between(log2_freqs, amps[i, 0]**2, edgecolor='none')
        ...
        # axes refinement
        ax_x.set_ylabel(f'({chr(ord("a") + i)})', rotation='horizontal', va='center', fontsize=12, labelpad=12)
        for ax in axs:
            ax.set_yticks([])
            for spine in ax.spines:
                ax.spines[spine].set_color(spine_color)
        if row == nrows - 1:
            ...
            ax_p.set_xticklabels(freq_ticklabels)
            for ax in axs:
